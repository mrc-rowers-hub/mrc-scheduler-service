...
public interface SessionRepository extends JpaRepository<Session, Long> {
    @Query("SELECT s FROM Session s WHERE s.day = :day AND s.startTime = :startTime AND s.endTime = :endTime AND s.squad = :squad AND s.level = :level AND s.sessionType = :sessionType")
            List<Session> findByCriteria(@Param("day") String day,
