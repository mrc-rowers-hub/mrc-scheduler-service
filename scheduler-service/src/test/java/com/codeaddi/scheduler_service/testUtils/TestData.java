...
public class TestData {
    ...
    public static  Session validSession = Session.builder().id(validId).day("MONDAY").startTime(LocalTime.NOON).endTime(LocalTime.MIDNIGHT).squad(Squad.DEVELOPMENT).level(RowerLevel.DEVELOPMENT).sessionType(SessionType.WATER).build();
    public static  Session validSessionReplacement = Session.builder().id(validId).day("MONDAY").startTime(LocalTime.NOON).endTime(LocalTime.MIDNIGHT).squad(Squad.WOMENS).level(RowerLevel.NOVICE).sessionType(SessionType.WATER).build();
